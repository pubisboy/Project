...
<div>
	<form action="/admin/management/counsel/counsel_user_list.ja"
	...
	<tbody>
		<c:forEach items="${list }" var="i" varStatus="vs">
			<tr>
				...
				<td>${i.NUM }</td>
				<td align="left"><a
					href="/admin/management/counsel/counsel_user_detail.ja?num=${i.NUM }&p=${params.p }&type=${params.type }&value=${params.value}&state=${params.state}&reply=${params.reply}&sort=${params.sort}">${i.TITLE }</a></td>
				<td>${i.USER_ID }</td>
...
<div align="center">
	<c:if test="${paging.startPageNo ne paging.firstPageNo }">
		<a
			href="/admin/management/counsel/counsel_user_list.ja?p=${paging.startPageNo - 1}&type=${params.type }&value=${params.value}&state=${params.state}&reply=${params.reply}&sort=${params.sort}">&lt;</a>
	</c:if>

	<c:forEach begin="${paging.startPageNo }" end="${paging.endPageNo }"
		var="i">
		<a
			href="/admin/management/counsel/counsel_user_list.ja?p=${i}&type=${params.type }&value=${params.value}&state=${params.state}&reply=${params.reply}&sort=${params.sort}"
			${paging.currentPageNo eq i ? 'style="font-weight:bold"' : '' }>${i }</a>
	</c:forEach>

	<c:if test="${paging.endPageNo ne paging.finalPageNo }">
		<a
			href="/admin/management/counsel/counsel_user_list.ja?p=${paging.endPageNo + 1}&type=${params.type }&value=${params.value}&state=${params.state}&reply=${params.reply}&sort=${params.sort}">&gt;</a>
	</c:if>
</div>
